...
export function Name() { return "Patriot Viper RAM"; }
...
export function Scan(bus) {
	...
	for(const addy of addressList) {
		...
		if(bus.ReadByte(addy, 0x00) === 0x23) {
			bus.log("Found Potential Patriot Viper RAM at: " + addy, {toFile: true});

			const registerstoReadFrom	= [0x40, 0x41, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68];
			const registerResponses		= [0x85, 0x02, 0x4D, 0x49, 0x43, 0x53, 0x59, 0x53, 0x5F, 0x44];

			// Send test write for control on Viper RAM
			...
			for(let bytes = 0; bytes < registerResponses.length; bytes++) {
				...
				if(response !== registerResponses[bytes]) {
					bus.log(`Potential Patriot Viper RAM returned: ${response}, expected: ${registerResponses[bytes]} for register: ${registerstoReadFrom[bytes]}`, {toFile: true});
					validResponse = false;
				}
			}

			if(validResponse) {
				bus.log(`Patriot Viper RAM found at: ${addy}`, {toFile: true});
				validSticks++;
			}
		}else{
...
export function Initialize() {
	Viper.writeRegister(0xFF, [0xFF, 0xFF, 0xFF]);
	Viper.writeRegister(0x03, [0x07, 0x00, 0x00]);
	Viper.writeRegister(0x01, [0x04, 0x00, 0x00]);
}
...
function sendColors(overrideColor) {
	...
	for(let iIdx = 0; iIdx < vLeds.length; iIdx++) {
		...
		Viper.writeRegister(Viper.registers[iIdx], color); // LED Color
		Viper.writeRegister(0x35, [0x01, 0x00, 0x00]); // Submit color
		device.pause(1); // Chip can't handle too much commands and flickers a lot more if removed
	}
}

class PatriotViperController {
	constructor() {
		this.registers = [0x30, 0x31, 0x32, 0x33, 0x34];
	}
...
const Viper = new PatriotViperController();
...
export function ImageUrl() {
	return "https://assets.signalrgb.com/devices/brands/patriot/ram/viper.png";
}
