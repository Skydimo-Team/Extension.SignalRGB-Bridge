...
export function Shutdown(SystemSuspending) {
	Corsair.CloseHandle(0, 1);
	Corsair.CloseHandle(1, 1);
...
function fetchLinkDevices() {
	//Read two packets for people with more devices.
	let childDevicePacket = Corsair.ReadFromEndpointMultiple(0, 0x36, 2, 0x01);
	...
	for(let bytes = 8; bytes < childDevicePacket.length; bytes++) {
		if(childDevicePacket[bytes] === 0x00 && childDevicePacket[bytes + 1] === 0x00 && childDevicePacket[bytes + 2] !== 0x00 && childDevicePacket[bytes + 4] === 0x00 && childDevicePacket[bytes + 5] === 0x00) {
			...
			if(childDevicePacket[bytes + 2] === 6) {
				...
				if(childDevicePacket[bytes + 2] === 25) { skipXD5 = true; }

				if(childDevicePacket[bytes + 2] === 14 && skipXD5) {
					device.log("Found fake XD5. Skipping.", { toFile : true });
					//If a user FOR SOME REASON has both an XD5 and XD6 we should not break that.
					skipXD5 = false;
					continue;
				}
