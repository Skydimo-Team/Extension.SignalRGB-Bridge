...
function SendChannel(Channel, overrideColor) {
	...
	const RedChannelData = ColorData[0].map( x => x * BrightnessLimiter);
	const GreenChannelData = ColorData[1].map( x => x * BrightnessLimiter);
	const BlueChannelData = ColorData[2].map( x => x * BrightnessLimiter);
...
export function ImageUrl() {
	return "https://assets.signalrgb.com/devices/brands/corsair/misc/lt100.png";
}
