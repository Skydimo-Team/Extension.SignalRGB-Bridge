...
export function ControllableParameters() {
	return [
		...
		},
		{
			...
			description:
				"Sets the Hardware effect to be used when SignalRGB is not controlling this device",
			type: "combobox",
			values: ["Static", "Nollie"],
			default: "Static",
		},
	];
}
...
function SendChannel(Channel, overrideColor) {
	...
	for (let CurrPacket = 0; CurrPacket < NumPackets; CurrPacket++) {
		const packet = [ChannelIndex[Channel]];
	...
	for (const ledCount of inputArray) {
		if (ledCount !== 0) {
			if (
				...
				currentGroupSum + ledCount <= maxLedsPerGroup
			) {
...
export function ImageUrl() {
	return "https://assets.signalrgb.com/devices/brands/nollie/controllers/Nollie16V3.png";
}
