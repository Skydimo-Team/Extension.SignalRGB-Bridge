...
export function ProductId() { return [0x1AA6];}
...
const ConfigurationOverrides = {
	...
	"ROG MAXIMUS Z790 APEX ENCORE": {MainboardCount: 3, ARGBChannelCount:2, RGBHeaderCount: 1}
};
...
const polymoDevice =
{
	Names : [ "ROG Logo LED 1", "ROG Logo LED 2", "ROG Logo LED 3", "ROG Logo LED 4", "ROG Logo LED 5", "ROG Logo LED 6", "ROG Logo LED 7", "ROG Logo LED 8", "ROG Logo LED 9", "ROG Logo LED 10", "ROG Logo LED 11", "ROG Logo LED 12", "ROG Logo LED 13", "ROG Logo LED 14", "ROG Logo LED 15",
		"Bottom LED 1", "Bottom LED 2", "Bottom LED 3", "Bottom LED 4", "Bottom LED 5", "Bottom LED 6", "Bottom LED 7", "Bottom LED 8", "Bottom LED 9", "Bottom LED 10" ],
	Positions : [ [5, 0], [5, 1], [5, 2], [5, 3], [5, 4], [5, 5], [5, 6], [5, 7], [5, 8], [5, 9], [5, 10], [5, 11], [5, 12], [5, 13], [5, 14],
...
function CreatePolymoSubdevice() {
	if(DeviceInfo.PolymoSupport) {
		...
		device.setSubdeviceName("Polymo", `${MotherboardName} - Polymo Panel`);
		device.setSubdeviceSize("Polymo", 10, 16);
...
function sendPolymoColors(overrideColor) {
	...
	StreamDirectColors(3, RGBData, polymoDevice.Positions.length);
}
...
function CreateMainBoardLeds(){
	...
	device.setSize([Math.max(DeviceInfo.MainChannelLedCount, 1), 1]);
...
function SendARGBChannel(ChannelIdx, overrideColor) {
	//Fetch Colors
...
function ParseConfigTable(){
	...
	for(const config in ConfigurationOverrides){
		...
		if(DeviceInfo.Model.localeCompare(config) == 0 || MotherboardName == config){
	...
	DeviceInfo.RGBHeaderCount = DeviceInfo.ConfigTable[ASUS_CONFIG_12V_HEADERS] - 1;
	...
	if(DeviceInfo.ConfigTable[ASUS_CONFIG_12V_HEADERS] === 3) //Weird offset edge case.
	{
		...
		DeviceInfo.MainChannelLedCount = Math.max(DeviceInfo.ConfigTable[ASUS_CONFIG_MAINBOARD_LEDS], 0);
		...
		DeviceInfo.RGBHeaderCount = Math.max(DeviceInfo.ConfigTable[ASUS_CONFIG_12V_HEADERS] - 1, 0);
		device.log(`12V Header Count ${DeviceInfo.RGBHeaderCount} `, {toFile: true});
	}
