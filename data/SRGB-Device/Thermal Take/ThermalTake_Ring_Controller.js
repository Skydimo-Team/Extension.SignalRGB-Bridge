...
export function ProductId() { return [0x2135, 0x2136, 0x2137, 0x2143, 0x2144]; } //will need to be changed based on PID shown in USB/PCI Info
...
function sendDirectPacket(channel, data) {
	for(let i = 0; i < 2; i++) {
		const byte = i === 0 ? 0x01 : 0x02;
		const header = [0x00, 0x32, 0x52, channel + 1, 0x24, 0x03, byte, 0x00];
		const packet = header.concat(data.splice(0, 57));
